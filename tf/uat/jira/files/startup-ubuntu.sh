...
# apt -y install curl lvm2 bind-utils telnet lsof unzip mailx nc krb5-workstation samba-common-tools sssd-ad


DEBIAN_FRONTEND=noninteractive /usr/bin/apt -y remove cylance-protect sshguard
DEBIAN_FRONTEND=noninteractive /usr/bin/apt -y install curl lvm2 telnet lsof unzip bsd-mailx netcat
DEBIAN_FRONTEND=noninteractive /usr/bin/apt -y install krb5-user samba sssd chrony

# /usr/bin/wget https://apt.puppetlabs.com/puppet5-release-xenial.deb -O /tmp/puppet5-release-xenial.deb
# dpkg -i /tmp/puppet5-release-xenial.deb
# DEBIAN_FRONTEND=noninteractive apt -y update
# DEBIAN_FRONTEND=noninteractive apt -y install puppet-agent

#/bin/systemctl unmask puppet
