...
### DNS via GCP via Corelogic US BIND forwarders sometimes does not work
# echo "10.72.66.114    yum.ad.corelogic.asia" >> /etc/hosts
echo "10.72.66.12     bpvlin01-puppetmaster.ad.corelogic.asia   puppet.ad.corelogic.asia   puppet" >> /etc/hosts

### Install dnsmasq to work around GCP Corelogic US BIND issues
...
# sed -i "s/^#no-negcache/no-negcache/" /etc/dnsmasq.conf
# sed -i "s/^#cache-size/cache-size=0/" /etc/dnsmasq.conf

cat << +++ > /etc/dnsmasq.d/00-cla.conf
listen-address=127.0.0.1
bind-interfaces
no-negcache
cache-size=0

server=/corelogic.asia/10.72.10.20
server=/corelogic.asia/10.72.10.21

server=/rpdata.local/10.72.10.20
server=/rpdata.local/10.72.10.21
...
grep -iE '^nameserver.*127.0.0.1' /etc/resolv.conf || sed -i "s/^nameserver.*169.254.169.254/nameserver 127.0.0.1\nnameserver 169.254.169.254/" /etc/resolv.conf
